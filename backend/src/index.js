...
app.use(async (req, res, next) => {
  ...
  // الاستثناء الذهبي: السماح لطلبات الدخول، التسجيل، واستعادة المرور
...
app.post(['/forgot-password', '/api/forgot-password', '/api/api/forgot-password'], async (req, res) => {
  ...
  try {
    if (!email || !newPassword) return res.status(400).json({ error: 'الرجاء إدخال البريد الإلكتروني وكلمة المرور الجديدة' });
    const cleanEmail = email.trim().toLowerCase();
    ...
    await prisma.user.update({
      ...
      data: { password: newPassword } // تحديث كلمة المرور
    });
