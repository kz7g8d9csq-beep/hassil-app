...
function App() {
  ...
  // حالات تسجيل الدخول والإنشاء والاستعادة
  ...
  const handleAuthSubmit = async (e) => {
    ...
    if (authView === 'login') {
      ...
      try {
        ...
        setAuthView('login'); // العودة لشاشة الدخول بعد النجاح
        setAuthPassword(''); // مسح الباسورد
      ...
      <div style={{ display: 'flex', height: '100vh', fontFamily: 'Tahoma, Cairo, sans-serif', direction: 'rtl', background: '#fff' }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', padding: '40px', background: '#f8fafc', overflowY: 'auto' }}>
          <div style={{ width: '100%', maxWidth: '400px' }}>
            ...
            <p style={{ margin: '0 0 30px 0', color: theme.textMuted, fontSize: '13px' }}>
              {authView === 'forgot' ? 'أدخل بريدك الإلكتروني وكلمة المرور الجديدة لاعتمادها.' : (authView === 'login' ? 'أدخل بياناتك للوصول إلى مساحة العمل.' : 'قم بتأسيس شركتك في محور للبدء في إدارة أعمالك.')}
            </p>
            ...
            <form onSubmit={handleAuthSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <button type="submit" disabled={isLoading} style={{ background: theme.primary, color: '#fff', padding: '14px', borderRadius: '8px', border: 'none', fontSize: '16px', fontWeight: 'bold', cursor: isLoading ? 'not-allowed' : 'pointer', display: 'flex', justifyContent: 'center', gap: '10px', alignItems: 'center' }}>
                <span>{isLoading ? 'جاري المعالجة...' : (authView === 'forgot' ? 'إعادة التعيين' : (authView === 'login' ? 'دخول آمن' : 'تأسيس المنشأة'))}</span>
                {!isLoading && <span>←</span>}
              </button>

              <div style={{ textAlign: 'center', marginTop: '10px', fontSize: '13px' }}>
                {authView === 'forgot' ? (
                  <span onClick={() => setAuthView('login')} style={{ color: theme.primary, fontWeight: 'bold', cursor: 'pointer', textDecoration: 'underline' }}>العودة لتسجيل الدخول</span>
                ) : (
                  <>
                    <span style={{ color: theme.textMuted }}>{authView === 'login' ? 'ليس لديك حساب منشأة؟ ' : 'لديك مساحة عمل مسبقاً؟ '}</span>
                    <span onClick={() => setAuthView(authView === 'login' ? 'register' : 'login')} style={{ color: theme.primary, fontWeight: 'bold', cursor: 'pointer', textDecoration: 'underline' }}>
